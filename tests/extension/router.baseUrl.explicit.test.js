...
describe('app.navigate', () => {
    it('should return a promise that resolves with info about the navigation', async () => {
        await expect(app.navigate('/base/test-1')).resolves.toEqual({
    ...
    it('should report navigated flag to false when it does not trigger a page load', async () => {
        const { id } = await app.navigate('/base/test-nested/default');
        expect(app.path).toEqual('/base/test-nested/default');

        await expect(app.navigate('/base/test-nested/default')).resolves.toEqual({
            id: id,
            path: '/base/test-nested/default',
            navigated: false,
            redirected: false,
    ...
    it('should resolve to same result when navigating to the same path immediately', async () => {
        ...
        const promise2 = app.navigate('/base/test-1');
    ...
    it('should cancel previous navigation before page load', async () => {
        ...
        app.beforePageEnter(defunctAfterTest(() => {
            resolve(app.navigate('/base/test-2'));
        }));
        await expect(promise1).rejects.toBeErrorWithCode('brew/navigation-cancelled');
        await expect(promise2).resolves.toEqual(objectContaining({ path: '/base/test-2' }));
        expect(app.path).toEqual('/base/test-2');
    ...
    it('should remove cancelled navigation from history', async () => {
        catchAsync(app.navigate('/base/test-1'));
    ...
    it('should report redirected flag for previous navigation when redirecting before page load', async () => {
        ...
        app.beforePageEnter(defunctAfterTest(() => {
            resolve(app.navigate('/base/test-2', true));
        }));
        await expect(promise1).resolves.toEqual(objectContaining({ path: '/base/test-2', redirected: true }));
        await expect(promise2).resolves.toEqual(objectContaining({ path: '/base/test-2' }));
        expect(app.path).toEqual('/base/test-2');
    });
    ...
    it('should continue page load if error is thrown in navigate event', async () => {
        ...
        bindEvent(app, 'navigate', cb);
        await expect(app.navigate('/base/test-1')).resolves.toEqual(objectContaining({
            navigated: true
        }));
    ...
    xit('should prevent infinite redirection loop in navigate event', async () => {
        ...
        bindEvent(app, 'navigate', e => {
            ...
            if (e.pathname === '/base/test-1') {
                app.navigate('/base/test-2', true);
            } else {
                app.navigate('/base/test-1', true);
            }
        });
        await expect(app.navigate('/base/test-1')).resolves.toEqual(objectContaining({ path: '/base/test-1' }));
        expect(i).toEqual(2);
    });
    ...
    it('should prevent excessive navigation during navigate event', async () => {
        ...
        bindEvent(app, 'navigate', e => {
            if (++i <= 50) {
                promises.push(muteRejection(app.navigate('/base/baz/' + i)));
            }
        });
        promises.push(muteRejection(app.navigate('/base/baz/' + i)));
        await delay(100);
    ...
    it('should prevent excessive redirection during navigate event', async () => {
        ...
        bindEvent(app, 'navigate', e => {
            if (++i <= 50) {
                lastPromise = app.navigate('/base/baz/' + i, true);
            }
        });
        await expect(app.navigate('/base/baz/' + i)).resolves.toEqual(objectContaining({ redirected: true }));
    ...
    it('should resume navigation when user allowed leaving', async () => {
        ...
        await expect(app.navigate('/base/')).resolves.toEqual(_);
        expect(cb).toBeCalledTimes(1);
    });
    ...
    it('should encode path properly', async () => {
        await app.navigate('/base/baz/foo%2f ¥');
        expect(app.path).toEqual('/base/baz/foo%2F%20%C2%A5');
    });
    ...
    it('should push a history entry when query string or hash is changed', async () => {
        const { id: id1 } = await app.navigate('/base/test-1');
        const storage = app.historyStorage.current;

        await expect(app.navigate('/base/test-1?a=1')).resolves.toEqual({
            id: id1,
            path: '/base/test-1?a=1',
            navigated: false,
            redirected: false,
            originalPath: null
        });
    ...
    it('should carry data when navigation did not happen', async () => {
        ...
        await expect(app.navigate('/base/test-2')).resolves.toMatchObject({ navigated: false });
    ...
    it('should pass data to navigate and beforepageload event', async () => {
        ...
        const { id: newStateId } = await app.navigate('/base/test-1', false, data);
        const payload = {
            navigationType: 'navigate',
            pathname: '/base/test-1',
    ...
    it('should emit pushstate event when only query string or hash has changed', async () => {
        ...
        const payload = {
            navigationType: 'navigate',
            pathname: '/base/test-1?a=1',
            oldPathname: '/base/test-1',
...
describe('app.back', () => {
    ...
    it('should update path to previous snapshot', async () => {
        await app.navigate('/base/test-1');
        await app.navigate('/base/test-1#a=1');
        expect(app.path).toBe('/base/test-1#a=1');
    ...
    it('should emit popstate event when returned to previous snapshot of the same page', async () => {
        const { id: newStateId } = await app.navigate('/base/test-1');
        app.snapshot();
    ...
    it('should emit popstate event after app.path has updated', async () => {
        await app.navigate('/base/test-1');
        app.navigate('/base/test-1?foo=bar');

        const cb = mockFn(() => app.path);
        cleanupAfterTest(app.on('popstate', cb));
        await app.back();
        expect(cb).toReturnWith('/base/test-1');
    });
...
describe('app.snapshot', () => {
    ...
    it('should have no effect when navigating', async () => {
        ...
        const result = app.navigate('/base/test-1');
        const stateId = history.state;
    ...
    it('should clone history storage from previous state', async () => {
        const data = {};
        const { id } = await app.navigate('/base/test-1');
...
describe('app.path', () => {
    ...
    it('should trigger navigation when being set with new value', async () => {
        const cb = mockFn();
        bindEvent(app, 'navigate', cb);
        app.path = '/base/test-1';
        await delay(100);

        expect(cb).toBeCalledWith(objectContaining({ type: 'navigate', pathname: '/base/test-1' }), _);
        expect(app.path).toEqual('/base/test-1');
    });
});
...
describe('app.previousPath', () => {
    ...
    it('should return path of previous page load in history stack', async () => {
        await app.navigate('/base/test-1');
        expect(app.previousPath).toEqual('/base');
        await app.navigate('/base/test-2');
        expect(app.previousPath).toEqual('/base/test-1');
        await app.back();
        expect(app.previousPath).toEqual('/base');
    });
});
...
describe('app.resolvePath', () => {
    it('should resolve relative path', async () => {
        await app.navigate('/base/foo/bar');
        expect(app.resolvePath('baz')).toEqual('/base/foo/bar/baz');

        await app.navigate('/base/foo/bar?foo=bar#baz');
        expect(app.resolvePath('baz')).toEqual('/base/foo/bar/baz');
    });

    it('should resolve ~/ path relative to min path of current route', async () => {
        await app.navigate('/base/foo/bar');
        expect(app.resolvePath('~/')).toEqual('/base/foo');
        expect(app.resolvePath('~/bar')).toEqual('/base/foo/bar');

        await app.navigate(initialPath);
        expect(app.resolvePath('~/', '/base/foo/bar')).toEqual('/base/foo');
        expect(app.resolvePath('~/bar', '/base/foo/bar')).toEqual('/base/foo/bar');
    });
    ...
    it('should resolve parameters against current route', async () => {
        await app.navigate('/base/foo/bar');
        expect(app.resolvePath('/base/foo/{id}')).toEqual('/base/foo/bar');
        expect(app.resolvePath('/base/{id}')).toEqual('/base/bar');
        expect(app.resolvePath('/base/{foo}')).toEqual('/base/null');

        await app.navigate(initialPath);
        expect(app.resolvePath('/base/foo/{id}', '/base/foo/bar')).toEqual('/base/foo/bar');
        expect(app.resolvePath('/base/{id}', '/base/foo/bar')).toEqual('/base/bar');
        expect(app.resolvePath('/base/{foo}', '/base/foo/bar')).toEqual('/base/null');
    });

    it('should remove trailing segments if optional parameter is empty', async () => {
        await app.navigate('/base/foo');
        expect(app.resolvePath('/base/foo/{id?}')).toEqual('/base/foo');
        expect(app.resolvePath('/base/foo/{id?}/baz')).toEqual('/base/foo/null/baz');
    ...
    it('should resolve to current path when given query string or hash only', async () => {
        await app.navigate('/base/foo?bar=baz#hash');
        expect(app.resolvePath('#foo')).toEqual('/base/foo?bar=baz#foo');
...
describe('app.route', () => {
    ...
    it('should return decoded value from pathname', async () => {
        await app.navigate('/base/baz/foo%2f%20%c2%a5bar');
    ...
    it('should cancel or replace previous navigation when parameter changes in observable callback [zeta-dom@>=0.5.10]', async () => {
        ...
        await expect(app.navigate('/base/foo/bar')).rejects.toBeErrorWithCode('brew/navigation-cancelled');
        await waitFor(() => expect(app.path).toBe('/base/foo/baz'));
    ...
    it('should normalize route parameter', async () => {
        ...
        await app.navigate('/base/foo/baz');
    ...
    it('should match path by params correctly', () => {
        expect(app.route.getPath({ id: 'bar' })).toEqual('/base/foo/bar');
        expect(app.route.getPath({ id: 'bar', remainingSegments: '/' })).toEqual('/base/foo/bar');
        expect(app.route.getPath({ id: 'bar', remainingSegments: '/baz' })).toEqual('/base/foo/bar');
        expect(app.route.getPath({ baz: 'baz' })).toEqual('/base/foo/baz');
        expect(app.route.getPath({ baz: 'baz', remainingSegments: '/' })).toEqual('/base/foo/baz');
        expect(app.route.getPath({ baz: 'baz', remainingSegments: '/baz' })).toEqual('/base/foo/baz/baz');
        expect(app.route.getPath({ test: 'quz', optional: 'another', remainingSegments: '/remaining' })).toEqual('/base/quz/another/remaining');
        expect(app.route.getPath({ test: 'quz', remainingSegments: '/000' })).toEqual('/base/quz');
    });
    ...
    it('should ignore query and hash when parsing route params', () => {
        ...
        expect(app.route.parse('/base/foo/bar?a=1&b=1')).toEqual(result);
        expect(app.route.parse('/base/foo/bar#a=1&b=1')).toEqual(result);
        expect(app.route.parse('/base/foo/bar#a=1&b=1?c=1')).toEqual(result);
    });

    it('should return object containing all params when path being parsed matches no routes', () => {
        expect(app.route.parse('/base/unknown')).toEqual({
            another: null,
            bar: null,
            baz: null,
            id: null,
    ...
    it('should handle remainingSegments with encoded characters', () => {
        expect(app.route.parse('/base/foo/baz/foo%2f%20%c2%a5bar')).toEqual(objectContaining({
...
describe('app.isElementActive', () => {
    ...
    it('should return true if the element has a direct match-path ancestor that matches current path', async () => {
        await app.navigate('/base/test-nested');
    ...
    it('should return true if the element has matched descendents', async () => {
        await app.navigate('/base/test-nested/default');
...
describe('app.historyStorage', () => {
    ...
    it('should update between navigate and beforepageload event', async () => {
        ...
        const promise = app.navigate('/base/test-1');
...
describe('app.page', () => {
    ...
    it('should expose information of current page', async () => {
        const data = {};
        const { id, path } = await app.navigate('/base/test-1', false, data);
    ...
    it('should update between navigate and beforepageload event', async () => {
        ...
        verifyCalls(cb, [
            ...
            ['beforepageload event', objectContaining({ path: '/base/test-1' })]
        ]);
    });
    ...
    it('should return different object after redirection', async () => {
        ...
        await app.navigate('/base/test-2', true);
    ...
    it('should return different object after redirected back to previous page', async () => {
        ...
        await app.navigate('/base/test-2');
        const result = await app.navigate('/base/test-1', true);
        expect(app.page).not.toBe(page);
        expect(app.page).toMatchObject({
            pageId: result.id,
            path: '/base/test-1'
    ...
    it('should return different object after redirected back to previous page in beforepageload event', async () => {
        ...
        const result = await app.navigate('/base/test-2');
        expect(app.page).not.toBe(page);
        expect(app.page).toMatchObject({
            pageId: result.id,
            path: '/base/test-1'
    ...
    it('should return same object after redirected back to previous page in navigate event', async () => {
        ...
        const { navigated } = await app.navigate('/base/test-2');
    ...
    it('should clear navigation data', async () => {
        const cb = mockFn();
        const { id } = await app.navigate('/base/test-1', false, {});
...
describe('app.navigationType', () => {
    it('should return navigate after navigation', async () => {
        ...
        await app.navigate('/base/bar', true);
        expect(app.navigationType).toBe('navigate');
    });
...
describe('processPageChange', () => {
    ...
    it('should not set variables from set-var on already matched match-path element', async () => {
        await app.navigate('/base/test-var/child-1');
        const initialValue = getVar(div.root, 'parentVar');

        await app.navigate('/base/test-var/child-3');
    ...
    it('should reset own variables to null on previously matched match-path element and its now non-active descendents', async () => {
        ...
        await app.navigate('/base/test-var/child-2');
        expect(getVar(div.var, 'myVar')).not.toBeNull();
    ...
    it('should add hidden class to previously matched match-path element after animation has completed', async () => {
        await app.navigate('/base/test-anim');
    ...
    it('should not trigger event or animation for match-path element that has its state briefly changed', async () => {
        ...
        const promise = app.navigate('/base/test-2');
        expect(cb).toBeCalledTimes(1);
...
describe('mounted event', () => {
    ...
    it('should be emitted on first-time matched element before pageenter event', async () => {
        ...
        await app.navigate('/base/test-mounted');
        expect(cb).toBeCalledTimes(2);
...
describe('pageenter event', () => {
    it('should be emitted on newly matched element and bubble up', async () => {
        ...
        await app.navigate('/base/test-1');
        verifyCalls(cb, [
            [objectContaining({ type: 'pageenter', pathname: '/base/test-1', target: div.test1 }), div.test1],
            [objectContaining({ type: 'pageenter', pathname: '/base/test-1', target: div.test1 }), div.root],
        ]);
    });
    ...
    it('should play video and audio that is originally attributed as autoplay', async () => {
        ...
        await app.navigate('/base/test-autoplay');
...
describe('pageleave event', () => {
    it('should be emitted on previously matched element and bubble up', async () => {
        ...
        await app.navigate('/base/test-1');
        verifyCalls(cb, [
            [objectContaining({ type: 'pageleave', pathname: '/base', target: div.initial }), div.initial],
            [objectContaining({ type: 'pageleave', pathname: '/base', target: div.initial }), div.root],
        ]);
    });
    ...
    it('should emit reset event on form element', async () => {
        ...
        await app.navigate('/base/test-form');
        await app.navigate(initialPath);
        verifyCalls(cb, [
